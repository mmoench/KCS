...
namespace KCS
{
    public class TrackedVessel
    {
        ...
        public List<ProtoCrewMember> cachedCrew = null;

        public int cachedCrewCount; // TODO: implement this
        public int cachedMedicCount; // TODO: implement this
        public int cachedOfficerCount; // TODO: implement this
        ...
        public static TrackedVessel CreateFromVessel(Vessel vessel)
        {
            ...
            trackedVessel.cachedCrew = new List<ProtoCrewMember>();
...
//            trackedVessel.UpdateCachedValues(); // Initialize the tracking
        ...
        */
        ...
        // Kills the given crew member or all members of the crew, if no name is given. The reason is purely for logging.
        public void KillCrew(String kerbalName = null, String reason = "no reasons")
        {
            try
            {
                ...
                if (vessel.loaded)
                {
                    if (vessel.isEVA)
                    {
                        if (kerbalName == null || vessel.GetVesselCrew()[0].name == kerbalName)
                        {
                    ...
                    else
                    {
                        foreach (Part part in vessel.parts)
                        {
                            ...
                            {
                                if (kerbalName == null || crewMember.name == kerbalName)
                                {
                            ...
                            if (kerbalName == null) part.protoModuleCrew.Clear();
                            else part.protoModuleCrew.RemoveAll(k => k.name == kerbalName);
                        }
                        if (kerbalName == null) vessel.MurderCrew(); // This does not seem to work, but lets be safe here
                    }
                }
                else
                {
                    ...
                    foreach (ProtoPartSnapshot protoPart in vessel.protoVessel.protoPartSnapshots)
                    {
                        ...
                        {
                            ProtoCrewMember crewMember = protoPart.protoModuleCrew.Find(k => k.name == kerbalName);
                            if (crewMember != null)
                            {
                                killList.Add(crewMember);
                                protoPart.protoModuleCrew.Remove(crewMember);
                                //protoPart.protoCrewIndicesBackup.Clear(); // TODO: Not sure if we can simply leave this as is
                                protoPart.protoCrewNames.Remove(kerbalName);
                            }
                        }
                    }

                    // Kill all crew members we found:
                ...
                if (deadKerbalNames.Count > 0)
                {
                    ...
                    foreach (string deadKerbalName in deadKerbalNames)
                    {
                        if (!KCS.killList.Contains(deadKerbalName)) KCS.killList.Add(deadKerbalName);
                    }
                    ...
                    {
                        message = deadKerbalNames.Count.ToString() + " have died because of " + reason + ": " + String.Join(", ", deadKerbalNames.ToArray());
                    }
                    else
                    {
                        message = deadKerbalNames[0] + " has died because of " + reason + "!";
                    }
                    Debug.Log("[KCS] " + message);
            ...
            {
                Debug.LogError("[KCS] TrackedVessel.KillCrew(" + vessel.id + "): " + e.ToString());
            }
        }
    }
}
